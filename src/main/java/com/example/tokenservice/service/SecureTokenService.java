...
public class SecureTokenService {
    ...
    private final Set<String> renewedTokens = ConcurrentHashMap.newKeySet();
    ...
    public TokenInfo renewToken(String token) {
        if (token == null || token.isEmpty()) {
            throw new TokenRenewalException(TokenRenewalException.RenewalErrorCode.INVALID_TOKEN);
        }
        ...
        try {
            ...
            if (revokedTokens.contains(token)) {
                throw new TokenRenewalException(TokenRenewalException.RenewalErrorCode.TOKEN_REVOKED);
            }

            if (renewedTokens.contains(token)) {
                throw new TokenRenewalException(TokenRenewalException.RenewalErrorCode.TOKEN_REVOKED, "Token has already been renewed");
            }
            ...
            if (expireTime <= now) {
                throw new TokenRenewalException(TokenRenewalException.RenewalErrorCode.TOKEN_EXPIRED);
            }

            long timeUntilExpiry = expireTime - now;
            if (timeUntilExpiry > refreshWindow) {
                throw new TokenRenewalException(TokenRenewalException.RenewalErrorCode.NOT_IN_RENEWAL_WINDOW);
            }
