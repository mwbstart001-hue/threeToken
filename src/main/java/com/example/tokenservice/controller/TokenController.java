...
import com.example.tokenservice.dto.RenewTokenRequest;
import com.example.tokenservice.dto.RenewTokenResponse;
...
public class TokenController {
    ...
    @PostMapping("/renew")
    public ResponseEntity<RenewTokenResponse> renewToken(@RequestBody RenewTokenRequest request) {
        if (request == null || request.getToken() == null || request.getToken().isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(RenewTokenResponse.error("Token cannot be empty"));
        }

        try {
            TokenInfo tokenInfo = tokenService.renewToken(request.getToken());
            return ResponseEntity.ok(RenewTokenResponse.success(
                    tokenInfo.getToken(),
            ...
            return ResponseEntity.status(status)
                    .body(RenewTokenResponse.error(e.getMessage()));
        }
    }
}
