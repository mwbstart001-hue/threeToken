...
public class TokenControllerTest {
    ...
    @Test
    public void testRenewToken_Success() throws Exception {
        when(tokenService.renewToken("valid-access-token")).thenReturn(mockTokenInfo);

        mockMvc.perform(post("/api/token/renew")
                .content("{\"token\": \"valid-access-token\"}")
    ...
    @Test
    public void testRenewToken_EmptyToken() throws Exception {
        mockMvc.perform(post("/api/token/renew")
                .content("{\"token\": \"\"}")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
    ...
    @Test
    public void testRenewToken_Expired() throws Exception {
        when(tokenService.renewToken(anyString()))
                .thenThrow(new TokenRenewalException(TokenRenewalException.RenewalErrorCode.TOKEN_EXPIRED));
    ...
    @Test
    public void testRenewToken_Revoked() throws Exception {
        when(tokenService.renewToken(anyString()))
                .thenThrow(new TokenRenewalException(TokenRenewalException.RenewalErrorCode.TOKEN_REVOKED));

        mockMvc.perform(post("/api/token/renew")
                .content("{\"token\": \"revoked-token\"}")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.success").value(false));
    }
    ...
    @Test
    public void testRenewToken_NotInWindow() throws Exception {
        when(tokenService.renewToken(anyString()))
                .thenThrow(new TokenRenewalException(TokenRenewalException.RenewalErrorCode.NOT_IN_RENEWAL_WINDOW));

        mockMvc.perform(post("/api/token/renew")
                .content("{\"token\": \"not-in-window-token\"}")
                .contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    public void testRenewToken_Invalid() throws Exception {
        when(tokenService.renewToken(anyString()))
                .thenThrow(new TokenRenewalException(TokenRenewalException.RenewalErrorCode.INVALID_TOKEN));
